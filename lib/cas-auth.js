...
exports.configureCas = function (app, config) {
    ...
    app.post('/auth/cas/login', rawParser, function (req, res) {
        ...
        xml.parseString(req.body.logoutRequest, function (err, logoutRequestt) {
            if (!err) {
                if (logoutRequestt['samlp:LogoutRequest'] && logoutRequestt['samlp:LogoutRequest']['samlp:SessionIndex'] && logoutRequestt['samlp:LogoutRequest']['samlp:SessionIndex'].length) {
                        ...
                        if (err) {
                            ...
                            for (var sid in sessions) {
                                ...
                                if (sessions[sid].cas_ticket && sessions[sid].cas_ticket === logoutTicket) {
                                    console.log("destroy session", sessions[sid].cas_user_loginName)
                    ...
                    console.log("can't parse logoutTicket.")
                }
            }
        });
